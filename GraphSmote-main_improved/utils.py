...
# def generate_embeddings(model, features, edge_index):
#     with torch.no_grad():
...
def recon_upsample(embed, labels, idx_train, adj=None, portion=1.0, im_class_num=3):
    ...
    for i in range(im_class_num):
        ...
        for j in range(c_portion):
            ...
            new_labels = labels.new_full((chosen.shape[0],), c_largest - i)
            idx_new = torch.arange(embed.shape[0], embed.shape[0] + chosen.shape[0], dtype=torch.long)
            idx_train_append = idx_train.new(idx_new)
